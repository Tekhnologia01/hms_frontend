    ...
    // if (paymentMode === "Cash") {
      try {
        ...
        const response = await axios.put(
          `${process.env.REACT_APP_API_URL}/bill/completebill`,
          formData,
          {
            params: {
              bill_id: details?.bill_id,
        ...
        console.log(e);
        showToast(
          e.response?.data?.message || "Error while completing payment!",
          "error"
        );
      }
    // } else if (paymentMode === "Online Mode") {
    //   showToast("Online payment is not yet implemented. Please select Cash.", "info");
    // }
  };
    ...
    <div className="mx-lg-4 m-3 pb-3">
      ...
      {details && (
        <Container className="p-2 p-lg-4">
          <Card style={{ border: "1px solid #EAECF0" }}>
            <Card.Body className="p-0">
              ...
              <Row className="my-2 p-3">
                <Col md={6}>
                  <p>
                    <strong>Patient Name : </strong> {details?.patient_name}
                  </p>
                  <p>
                    <strong>Disease : </strong> {details?.disease}
                  </p>
                  <p>
                    <strong>Consulting Dr. : </strong> {details?.doctor_name}
                  </p>
                </Col>
                <Col md={6} className="text-end">
                  <p>
                    <strong>UH I'd Number : </strong> {details?.uh_id}
                  </p>
                  <p>
                    <strong>Receipt No. : </strong> {details?.receipt_number}
                  </p>
                  <p>
                    <strong>Receipt Date : </strong> {details?.bill_date?.split("T")[0]}
                  </p>
                </Col>
              </Row>
              <hr color="#475467" className="mx-3" />
              ...
              <Table bordered style={{ borderColor: "#EAECF0" }} className="mb-0">
                <thead>
                  <tr>
                    <th className="text-center p-3" style={{ background: "#F9FAFB" }}>
                      No
                    </th>
                    <th className="text-center p-3" style={{ background: "#F9FAFB" }}>
                      Summary
                  ...
                  <tbody>
                    ...
                    {allCharges.map((charge, index) => (
                      <tr key={index}>
                        ...
                        <td className="text-center p-md-3 p-2">{charge?.quantity}</td>
                        <td className="text-center p-md-3 p-2">
          ...
          <div className="d-flex justify-content-end mt-4">
            <Button variant="outline-secondary" className="me-3" onClick={handelViewBill}>
              Print
            </Button>
            {details?.appo_biil_status === "Pending" && (
              <CommanButton
      ...
      <Modal show={showPaymentModal} onHide={() => setShowPaymentModal(false)}>
        <Modal.Header closeButton>
          <Modal.Title className="text-center" style={{ color: "#1E959B" }}>
            Payment Successful!
          </Modal.Title>
        </Modal.Header>
        <Modal.Body className="text-center p-4">
