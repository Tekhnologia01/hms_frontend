...
//             return Math.ceil((now - startDate) / (24 * 60 * 60)); // Convert to days
//         }
...
//             console.log('Total amount123: ' + totalAmount )
...
//             formData.append("bill_total_amount", totalAmount);
//             formData.append("ipd_id", details.ipd_id);
...
//             //     `${process.env.REACT_APP_API_URL}/bill/upload-ipd-bill`,
//             //     formData,
//             //     {
//             //         headers: {
...
//                                                 <td>Room Charge (Type {room.room_type})</td>
//                                                 <td>{new Date(room.start_date * 1000).toLocaleDateString()}</td>
...
//                                             <td>{new Date(charge.charge_date * 1000).toLocaleDateString()}</td>
...
//                                             <td>Doctor Visit - {visit.doctor_name}</td>
//                                             <td>{new Date(visit.visit_date * 1000).toLocaleDateString()}</td>
...
//                         <Button variant="outline-secondary" className="me-3" onClick={handlePrint} disabled={!details?.bill_report}>
//                             Print
//                         </Button>
//                         <CommanButton
...
function IpdBill() {
    ...
    const isAmountEqual = Math.abs(totalAmount - totalDeposit) < 0.01;
    const isBillNotPaid = details?.bill_status == 0;
    
    // Enable button only if both conditions are true
    const shouldEnableButton = isAmountEqual && isBillNotPaid;
