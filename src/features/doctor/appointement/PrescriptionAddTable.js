...
const AddPrescriptionTable = ({ appointmentId, ipd_id, rows, setRows, role, appointmentData, isIPD }) => {
        ...
        if (isIPD && ipd_id) {
            try {
                if (row?.Prescription_Id) {
                    ...
                    const response = await axios.post(`${process.env.REACT_APP_API_URL}/prescription/ipd_add`, row, {
                        params: {
                            ipd_id: ipd_id,
                            created_by: userId
                        }
                ...
                showToast(error?.response?.data?.message ? error?.response?.data?.message : "Failed to delete prescription.", "error");
            }
        } else {
            try {
                if (row?.Prescription_Id) {
                    const response = await axios.put(`${process.env.REACT_APP_API_URL}/prescription/update_prescription`, row, {
                        params: {
                ...
                showToast(error?.response?.data?.error ? error?.response?.data?.error : "Failed to delete preset", "error");
            }
        }
    ...
    const handleDeleteRow = async (index, row) => {
        try {
            if (isIPD && ipd_id) {
                if (row?.Prescription_Id) {
                    const response = await axios.delete(`${process.env.REACT_APP_API_URL}/prescription/delete_ipd`, {
                        params: {
                            prescription_id: row?.Prescription_Id
                        }
                    },
                ...
                if (row?.Prescription_Id) {
                    const response = await axios.delete(`${process.env.REACT_APP_API_URL}/prescription/delete`, {
                        params: {
                            prescription_id: row?.Prescription_Id
                        }
                    },config);
            ...
            showToast(error?.response?.data?.error ? error?.response?.data?.error : "Failed to delete prescription", 'error');
        }
    };
    ...
    return (
        <>
            <div className="row p-2 pt-0 m-0">
                <div className="fw-bold fs-5 col-md-6">Add Prescription</div>
