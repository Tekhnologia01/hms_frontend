...
const AddPrescriptionTable = ({ appointmentId, ipd_id, rows, setRows, role, appointmentData, isIPD }) => {
    ...
    const handleChange = (index, field, value) => {
        ...
        newRows[index][field] = value;
        if (field === "frequency" || field === "days") {
            // Calculate total frequency count (e.g., 1-1-1 = 3, 1-0-1 = 2)
            if (field === "frequency") {
                const freqParts = value.split("-").map(Number);
                const totalFreq = freqParts.reduce((sum, num) => sum + num, 0);
                newRows[index].quantity = totalFreq * (newRows[index].days || 0);
            } else {
                const freqParts = newRows[index].frequency.split("-").map(Number);
                const totalFreq = freqParts.reduce((sum, num) => sum + num, 0);
                newRows[index].quantity = totalFreq * value;
            }
        }
        setRows(newRows);
    };
