...
function Departments() {
  ...
  const handleDeleteDepartment = async () => {
    try {
      const response = await axios.delete(
        `${process.env.REACT_APP_API_URL}/department/delete/${departmentToDelete?.department_id}`,
        config
      );
      if (response.status) {
        getDepartments();
      ...
      >
        ...
        <div className="mx-3">
          <p>
            ...
            <strong>{departmentToDelete?.department_name}</strong> department?
          </p>
        </div>
        <div className="m-3 d-flex align-items-center gap-3 justify-content-end">
          <Button
          ...
          <CommanButton onClick={handleDeleteDepartment} label="Delete" />
        </div>
      </Modal>
    </div>
  );
